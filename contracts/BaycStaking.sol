...
import './BaseApeToken.sol';
import './BaseApeNFT.sol';

contract BaycStaking {
    uint totalStaked;
    ...
    constructor(BaseApeToken _token, BaseApeNFT _nft) {
        token = _token;
        nft = _nft;
    }
    ...
    function _claim(
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            require(
                ...
                'Not the owner, you cannot claim rewards'
            );

            uint stakingStartTime = thisStake.stakingStarted;

            earned =
                ((block.timestamp - stakingStartTime) * rewardPerHour) /
                3600;
            totalEarned += earned;
